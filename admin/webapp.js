...
$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    ...
    "columns": [
      { "data": "rank" },
      { "data": "companyName",   "sClass": "companyName" },
      { "data": "street" },
      { "data": "postnr",        "sClass": "integer" },
      { "data": "city",    "sClass": "integer" },
      { "data": "description",      "sClass": "integer" },
      { "data": "website",     "sClass": "integer" },
      { "data": "facebook" },
  ...
  $(document).on('click', '#add_company', function(e){
    ...
    $('#form_company #id').val('');
    $('#form_company #companyName').val('');
    $('#form_company #street').val('');
    $('#form_company #postnr').val('');
    $('#form_company #city').val('');
    $('#form_company #description').val('');
    $('#form_company #website').val('');
    $('#form_company #facebook').val('');
    show_lightbox();
  });
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    request.done(function(output){
      if (output.result == 'success'){
        ...
        $('#form_company .field_container label.error').hide();
        $('#form_company .field_container').removeClass('valid').removeClass('error');
        $('#form_company #id').val(output.data[0].rank);
        $('#form_company #companyName').val(output.data[0].companyName);
        $('#form_company #street').val(output.data[0].street);
        $('#form_company #postnr').val(output.data[0].postnr);
        ...
        $('#form_company #facebook').val(output.data[0].facebook);
        hide_loading_message();
        show_lightbox();
      } else {
        hide_loading_message();
    ...
    if (confirm("Are you sure you want to delete '" + companyName + "'?")){
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            hide_loading_message();
            show_message("Company '" + companyName + "' deleted successfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
